...
function criarLinhaLivro(livro) {
  ...
  linha.querySelector('[data-action="edit"]').addEventListener("click", function iniciarEdicaoDaLinha() {
    iniciarEdicao(livro);
  });
  linha.querySelector('[data-action="delete"]').addEventListener("click", function excluirDaLinha() {
...
async function lerErro(resposta) {
  try {
    ...
    if (Array.isArray(dadosErro.detail)) {
      return dadosErro.detail.map(function obterMensagem(dado) { return dado.msg; }).join("; ");
    }
...
async function carregarLivros() {
  ...
  try {
    ...
    livros.forEach(function adicionarLivro(livro) {
...
async function salvarLivro(evento) {
  ...
  try {
    ...
    mostrarMensagem(mensagemFormulario, idLivro ? "Livro atualizado com sucesso." : "Livro cadastrado com sucesso.", "success");
    limparFormulario();
...
async function excluirLivro(idLivro, titulo) {
  if (!window.confirm(`Deseja excluir o livro "${titulo}"?`)) {
    return;
  }
  try {
    const resposta = await fetch(`${URL_API}/livros/${idLivro}`, { method: "DELETE" });
    if (!resposta.ok) {
      throw new Error(await lerErro(resposta));
    }
